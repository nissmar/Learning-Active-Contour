...
parser.add_argument('-label', default=1, help='int: label (1, 2 or 3)')
parser.add_argument('-loss', default="CE", help='str: loss (CE or AC or MSE)')
parser.add_argument('-epochs', default=8, help='int: number of epochs')
parser.add_argument('-lr', default=10**-4, help='float: learning rate')
...
def evaluate(model, loader, sigmoid=False):
    ...
    for batch_idx, (data, target) in enumerate(tqdm(loader)):
        ...
        output = (torch.sigmoid(model(data))>0.5)*1.0 if sigmoid else (model(data)>0.5)*1.0 
...
for i in range(args.epochs):
    train(model, optimizer, i, train_loader, CE_loss, sigmoid) 
    evalu.append(evaluate(model, val_loader, sigmoid))
    print(evalu)


torch.save(model.state_dict(), 'models/unet{}_CE.pt'.format(args.label))